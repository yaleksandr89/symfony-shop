...
> [!IMPORTANT]
> Проект использует зафиксированный Chrome for Testing `150.0.7871.46`. Рекомендуемый способ получить архив — `git lfs pull`. Если Git LFS устанавливать не хочется, можно скачать архив исходного кода GitHub с включёнными LFS-объектами либо вручную взять нужную версию Chrome for Testing из официального источника. Точные ссылки, имя файла и SHA-256 приведены в [руководстве по запуску](docs/getting-started.md#git-lfs-и-chrome-for-testing).

> [!IMPORTANT]
> `.env.docker` передаётся PHP-контейнеру как настоящие переменные окружения. Если один и тот же ключ задан и там, и в `.env.local`, значение из `.env.docker` имеет приоритет. Подробная схема описана в [руководстве по конфигурации](docs/configuration.md#приоритет-переменных).
...
2. **Inertia.js и Vue 3.** Перевести взаимодействие серверной и клиентской частей на Inertia.js и Vue 3. Во время отдельного архитектурного аудита также пересмотреть локализацию и необходимость обязательного языкового префикса `/{_locale}` в URL; решение по этой части пока не принято.
...
- уязвимости — [политика безопасности](https://github.com/yaleksandr89/symfony-shop/security/policy).
