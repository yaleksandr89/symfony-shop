...
> [!IMPORTANT]
> Проект использует зафиксированный Chrome for Testing `150.0.7871.46`. Рекомендуемый способ получить архив — `git lfs pull`. После выхода следующего релиза проекта его ZIP-архив можно будет скачать со страницы Releases уже вместе с Chrome. Если Git LFS устанавливать не хочется до выхода релиза, нужную версию Chrome for Testing можно скачать напрямую из официального источника. Точные ссылки, имя файла и SHA-256 приведены в [руководстве по запуску](docs/getting-started.md#git-lfs-и-chrome-for-testing).
